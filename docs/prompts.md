...
### Phase 6.5 — 임시 조립 (수행 완료)

담당자 시연용. **완성된 컴포넌트만** 배치하고 나머지는 Phase 7 에서 채운다.

```
app/eval/[traceId]/page.tsx 를 서버 컴포넌트로 만든다.
backend 에서 GET /api/v1/evaluations/{traceId} 를 fetch 하고
현재까지 완성된 컴포넌트만 배치한다.

배치 (시안 레이아웃 기준):
- 헤더: 브레드크럼, 제목, 우측에 평가일 · trace ID · meta(모델 · 소요시간)
- 상단: GaugeRing(좌) + SummaryCards(우)
- 중단: QuestionTypeChart
- 하단: FailureTable
- 아직 없는 것(TargetApiCard, RecommendationCards, ActionPanel)은
  자리만 잡되 "구현 예정" 플레이스홀더를 두지 말고 그냥 비워둘 것

- loading.tsx, error.tsx, not-found.tsx 도 함께
- 백엔드 연결 실패 시 error.tsx 가 "백엔드(8000)가 실행 중인지 확인" 안내
- 루트 page.tsx 는 /eval/A492 로 리다이렉트 (미정 #1 확정 전 임시)
- 반응형: 1440 기준, 1024 이하 2단, 768 이하 1단

Phase 7 정식 조립에서 나머지를 채운다. 컴포넌트를 새로 만들지 말 것.
```

**핵심 판단**

- **빈 자리에 아무것도 두지 않는다.** "구현 예정" 카드를 깔면 화면 전체가
  미완성으로 보인다. 없으면 그냥 지금까지의 결과물로 보인다.
- **`cache: "no-store"`.** 평가 결과는 재실행마다 바뀐다. 캐시하면 재생성
  뒤에도 옛 수치가 남아, 이 제품의 핵심인 Before/After 가 거짓이 된다.
- **404 만 not-found 로 보낸다.** 연결 실패는 error.tsx 가 받아야
  "백엔드가 떠 있는지" 를 안내할 수 있다. "결과가 없다" 와 "백엔드가 죽었다" 는
  해야 할 일이 다르다.
- **시각은 변환하지 않는다.** `toLocaleString` 은 서버 시간대를 타서 응답에
  담긴 오프셋(+09:00)과 다른 시각이 찍힐 수 있다. ISO 문자열을 잘라 쓴다.
- **`'use client'` 는 error.tsx 에서만.** Next 의 에러 경계는 클라이언트
  컴포넌트여야 하고 `reset` 도 클라이언트에서만 동작한다.
- **루트 리다이렉트는 임시다.** 평가 대상 범위(#1)가 미정이라 무엇을 둘지
  정할 수 없다 — API 1개면 선택 화면이 필요 없고 서비스 전체면 목록이 먼저다.

**측정해서 알게 된 것 — `loading.tsx` 와 상태 코드는 맞바꿔야 한다**

`loading.tsx` 가 Suspense 경계를 만들어 스트리밍이 시작되면, 응답 상태가
이미 200 으로 확정된 뒤에 `notFound()` 나 에러가 발생한다.

| | `/eval/NOPE` | 백엔드 down |
|---|---|---|
| `loading.tsx` 있음 | **200** (화면은 not-found 가 맞게 뜸) | **200** |
| `loading.tsx` 없음 | **404** | **500** |

지시대로 `loading.tsx` 를 유지했다 — 평가가 수십 초 걸려 진행 중 화면이
실제로는 가장 자주 보이는 화면이다 (§9-4). 모니터링이 상태 코드를 본다면
Phase 7 에서 재검토한다 (open-questions #49).
...
|---|---|
| `/` | 307 → `/eval/A492` |
| `/eval/A492` | 200, 컴포넌트 4종 전부 렌더 |
| 헤더 meta | 평가일 `2026-07-22 11:38` · `A492` · `bge-m3` · `Hybrid (BM25 + 벡터), Top-3` · `48.2초` · `LLM 생성 · 사람 검수` |
| 게이지 | `Top-3 인식률 78.0%, 개선 필요` |
| 빈 자리 | 플레이스홀더 문구 없음 |

> `error.tsx` 는 클라이언트 에러 경계라 **초기 HTML 에는 어느 경우에도 나오지
> 않는다.** 브라우저에서는 스트림이 도착한 뒤 렌더된다. 브라우저 도구가 없어
> 눈으로는 확인하지 못했다 — 라우팅·상태 코드·빌드까지만 확인했다.
