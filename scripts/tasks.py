...
# 로컬 개발 서버는 **127.0.0.1 에 묶어둔다.**
#
# uvicorn 의 기본값이 127.0.0.1 이라 --host 를 주지 않는다. 컨테이너에서는
# 0.0.0.0 이 필요하지만(Dockerfile 참고) 그건 컨테이너 격리가 경계 역할을 해주기
# 때문이고, 로컬에서 0.0.0.0 으로 열면 같은 네트워크의 다른 기기에서
# 개발 서버에 그대로 접속할 수 있다. 카페나 공용 와이파이에서는 평가 fixture 와
# 사내 명세가 그대로 노출된다.
#
# 다른 기기에서 확인해야 할 일이 생기면 그때만 한시적으로 --host 를 붙인다.
