...
# 로컬 개발 서버는 **셋 다 127.0.0.1 에 묶어둔다.**
#
# 컨테이너에서는 0.0.0.0 이 필요하지만(Dockerfile 참고) 그건 컨테이너 격리가
# 경계 역할을 해주기 때문이다. 로컬에서 0.0.0.0 으로 열면 같은 네트워크의 다른
# 기기에서 개발 서버에 그대로 접속할 수 있다 — 카페나 공용 와이파이에서는
# 평가 fixture 와 명세가 그대로 노출된다.
#
# uvicorn 은 기본값이 127.0.0.1 이라 따로 주지 않는다.
# next dev 는 기본값이 모든 인터페이스라 명시해야 한다.
#
# 다른 기기에서 확인해야 할 일이 생기면 그때만 한시적으로 주소를 열어준다.
SERVICES: list[tuple[str, list[str], Path]] = [
    ...
    # next dev 는 uvicorn 과 달리 **기본값이 모든 인터페이스**다. 그냥 두면
    # 기동 로그에 "Network: http://192.168.x.x:3000" 이 찍히고, 같은 네트워크의
    # 다른 기기에서 그대로 접속된다. 그래서 명시적으로 묶어준다.
    # `npm run dev` 에 인자를 넘기려면 `--` 로 구분해야 next 까지 전달된다.
    (
        "frontend",
